...
imsize = 756 # 756 or 1024 -> size를 크게 키울수록 더 나은 성능을 보임
...
def get_transferred_file(content_filepath, style_filepath):
    ...
    style_image = image_loader(style_filepath)

    if content_image.shape != torch.Size([1, 3, imsize, imsize]):
        content_image = content_image[:, :-1, :, :]
    
    if style_image.shape != torch.Size([1, 3, imsize, imsize]):
        style_image = style_image[:, :-1, :, :]

    input_image = style_image.clone()

    cnn = models.vgg19(pretrained=True).features.to(device).eval()
    input_image = run_style_transfer(cnn, cnn_normalization_mean, cnn_normalization_std, content_image, style_image, input_image)

    to_PIL(input_image).save("./temp/transfer/result.jpg")
    return './temp/transfer/result.jpg'

def get_noise_file(content_filepath, style_transferred_filepath, p):
    content_image = image_loader(content_filepath)
    ...
    for _ in range(600):
        ...
        penalty = alpha * torch.clamp(perceptual_loss, min=0) # alpha * torch.clamp(perceptual_loss - p, min=0)
